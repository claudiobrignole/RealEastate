...
function resolveTargetTenantId(user: SessionUser | null): string {
  if (user?.role === 'super_admin') {
    return user.activeTenantId || user.tenantId || MASTER_TENANT_ID;
  }
...
export default function UsersSettingsClient({ initialUser }: UsersSettingsClientProps) {
  ...
  const [targetTenantId, setTargetTenantId] = useState(() => resolveTargetTenantId(initialUser));
      ...
      try {
        ...
        if (resolvedUser) {
          ...
          if (resolvedUser.role === 'super_admin') {
            setTargetTenantId(resolveTargetTenantId(resolvedUser));
          }
        }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      password,
      role,
      ...(currentUser?.role === 'super_admin' ? { tenantId: targetTenantId } : {}),
    };

    try {
      let res: { success: boolean; error?: string } | null = null;
      try {
        res = await createTenantUser(payload);
      } catch (serverActionErr) {
        console.warn('createTenantUser Server Action failed, using API fallback:', serverActionErr);
  ...
  const clientTenants = tenants.filter((t) => t.id !== MASTER_TENANT_ID);
  const activeTenantLabel =
    targetTenantId === MASTER_TENANT_ID
      ? `${MASTER_TENANT_NAME} (Master)`
      : tenants.find((t) => t.id === targetTenantId)?.name || 'Spazio selezionato';
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-6">
          <div className="bg-surface/50 p-4 border border-outline-variant rounded-xl flex items-center gap-3">
            ...
            <p className="text-xs text-on-surface-variant">
              {isSuperAdmin ? (
                <>
                  Spazio attivo: <strong className="text-on-surface">{activeTenantLabel}</strong>.
                  Puoi cambiarlo dal menu in alto o dal selettore nel form. Lo spazio <strong>Master</strong> è per il team ZeroAgenzia e i super admin.
                </>
              ) : (
                <>
                  Stai gestendo i membri del team per lo spazio attivo corrente.
                </>
              )}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <section className="bg-surface rounded-xl border border-outline-variant p-6 shadow-sm h-min">
              ...
              <form onSubmit={handleCreateUser} className="space-y-4">
                ...
                {isSuperAdmin && (
                  <div>
                    <label className="block text-xs font-semibold text-on-surface mb-1">Spazio di lavoro</label>
                    <select
                    ...
                    >
                      <option value={MASTER_TENANT_ID}>{MASTER_TENANT_NAME} (Master)</option>
                      {clientTenants.map((t) => (
                ...
                <div>
                  ...
                  >
                    ...
                    {isSuperAdmin && targetTenantId === MASTER_TENANT_ID && (
                      <option value="super_admin">Super Admin Master (Modifica tutto)</option>
                    )}
                  </select>
                </div>
      ...
      {activeTab === 'spaces' && isSuperAdmin && (
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <section className="bg-surface rounded-xl border border-outline-variant p-6 shadow-sm flex flex-col h-full min-h-[400px]">
            <h2 className="text-lg font-bold text-on-surface border-b border-outline-variant pb-2 mb-4">
              Spazi Clienti Attivi ({clientTenants.length + 1})
            </h2>
